...
class Agent:
    def __init__(self, theta_std, lamb, dim, action, p, B, T, K, delta, theta_true, num_action, mu_max):
        ...
        self.p_hat = p
        self.p_tilde = p
        self.freq_tilde = np.ones(self.K)
#         self.sigma_tilde = np.sqrt(2*self.p_tilde*(1-self.p_tilde)*np.log(2*self.K*self.T/self.delta)) + (7*np.log(2*self.K*self.T/self.delta))/3
#         self.sigma_hat = np.sqrt(2*self.p_tilde*(1-self.p_tilde)*np.log(2*self.K*self.T/self.delta)) + (7*np.log(2*self.K*self.T/self.delta))/3
#         self.sigma_tilde = [0.1]*self.K
#         self.sigma_hat = [0.1]*self.K
   ...
   # Algorithm 2 Online primal-dual algorithm with learning component
    ...
    def Barphi_hat(self,a, S):
        barphi = np.zeros((self.dim, 1))
        for i in range(self.K):
            phi = self.get_observation(a, S[i])
#             p_hat_draw = np.random.uniform(self.p_hat[i]-self.sigma_hat[i],self.p_hat[i]+self.sigma_hat[i],1)
            p_hat_draw = self.p_hat[i]
            barphi += phi*p_hat_draw
        return barphi
    
    def Barphi_tilde(self,a, S):
        barphi = np.zeros((self.dim, 1))
        for i in range(self.K):
            phi = self.get_observation(a, S[i])
#             p_tilde_draw = np.random.uniform(self.p_tilde[i]-self.sigma_tilde[i],self.p_tilde[i]+self.sigma_tilde[i],1)
            p_tilde_draw = self.p_tilde[i]
            barphi += phi*p_tilde_draw
        return barphi
        
    def q1(self, s_t):
        return 0.8 + 0.2 * s_t**2 
...
#             upper_hat[a] = np.matmul(barphi.T, theta_hat) + self.alpha_hat*np.sqrt(np.matmul(np.matmul(barphi.T, npla.inv(self.V_hat)), barphi))
            upper_hat[a] = np.matmul(barphi.T, theta_hat) + np.sqrt(np.matmul(np.matmul(barphi.T, npla.inv(self.V_hat)), barphi)) 
            #revealer with st
#             upper_tilde_st[a] = np.matmul(phi.T, theta_tilde) + self.alpha_tilde*np.sqrt(np.matmul(np.matmul(phi.T, npla.inv(self.V_tilde)), phi)) 
            upper_tilde_st[a] = np.matmul(phi.T, theta_tilde) + np.sqrt(np.matmul(np.matmul(phi.T, npla.inv(self.V_tilde)), phi)) 
        #print(upper_tilde)
    ...
    def update_parameter(self, s_t, X_t, phi_A, O_t, Y_t1, Y_t0, index):       
        ...
        if Y_t == 1:    
            ...
            self.p_hat = self.p_tilde
#             self.sigma_hat = self.sigma_tilde

    
        #update revealer
        self.V_tilde += np.matmul(phi_A, phi_A.T)
        self.b_tilde += X_t * phi_A
        self.alpha_tilde = np.sqrt(2*np.log(1/self.delta) + np.log(npla.det(self.V_tilde)/(self.lamb**self.dim)))+np.sqrt(self.lamb)*(npla.norm(self.theta_true))
...
class Two_Agent_pd2(Agent):
    ...
    def PrimalDual(self, u, v, tilde_a_t, hat_a_t, S, s_t):
        ...
        barphi_tilde = self.Barphi_tilde(tilde_a_t, S)
        barphi_hat = self.Barphi_hat(hat_a_t, S)
    ...
    def pick_action(self, s_t, S,  Y_t0, Y_t1):
        ...
        upper_tilde_st = np.zeros(self.num_action)
        for a in range(self.num_action):
            barphi_hat = self.Barphi_hat(a,S)
            barphi_tilde = self.Barphi_tilde(a,S)
            phi = self.get_observation(a, s_t)
            #revealer
#             upper_tilde[a] = np.matmul(barphi.T, theta_tilde) + self.alpha_tilde*np.sqrt(np.matmul(np.matmul(barphi.T, npla.inv(self.V_tilde)), barphi))
            upper_tilde[a] = np.matmul(barphi_tilde.T, theta_tilde) + np.sqrt(np.matmul(np.matmul(barphi_tilde.T, npla.inv(self.V_tilde)), barphi_tilde))
            #recommender
#             upper_hat[a] = np.matmul(barphi.T, theta_hat) + self.alpha_hat*np.sqrt(np.matmul(np.matmul(barphi.T, npla.inv(self.V_hat)), barphi))
            upper_hat[a] = np.matmul(barphi_hat.T, theta_hat) + np.sqrt(np.matmul(np.matmul(barphi_hat.T, npla.inv(self.V_hat)), barphi_hat)) 
...
#             upper_tilde_st[a] = np.matmul(phi.T, theta_tilde) + self.alpha_tilde*np.sqrt(np.matmul(np.matmul(phi.T, npla.inv(self.V_tilde)), phi)) 
        ...
        barphi_A_tilde = self.Barphi_tilde(A_tilde,S)/(1.5*self.mu_max)
        phi_A_tilde_st = self.get_observation(A_tilde_st, s_t)/(1.5*self.mu_max)        
    ...
    def update_beta_t (self, beta_t, t):
        self.beta_t = (1.2*beta_t)/(np.sqrt(t/5 + 1)+0.1)  #sqrt t
...
class Omniscient:
    ...
    def q1(self, s_t):
        return 0.8 + 0.2 * s_t**2  # Quadratic term for variation

    def q0(self, s_t):
        return 0.1 * s_t**2  # Smaller coefficients to ensure q1 > q0
    ...
    def omniscient_strategy(self, s_t_list, S, Y_t0_list, Y_t1_list):
        ...
        reward_omni_list = [0]*self.T # create a list of reward_t
...
def simu_regret(s, theta_std, lamb, q, action, B, T, K, delta, num_action, theta_true):
    ...
    p_0 = np.concatenate([np.random.uniform(0, 1, 7), np.random.uniform(6, 10, 3)])
    p_true = p_0/np.sum(p_0)
    p_init = np.ones(K)/K
#     p_init = p_true
    
    s_t_list = np.random.multinomial(1, p_true, T) #sequence
    index_t_list = np.argmax(s_t_list==1, axis=1)
    s_t_list = S[index_t_list]
    ...
    omniscient = Omniscient(q, action, p_true, B, c, T, K, num_action, theta_true)
...
#     cumu_regret_algorithm3 = np.ones(T)

    # saving o_t
    o_pd1_list = np.zeros(T)
    o_pd2_list = np.zeros(T)
#     o_pd3_list = np.zeros(T)
...
#     regret_algorithm3 = 0
    ...
    agent = Agent(theta_std, lamb, q, action, p_init, B, T, K, delta, theta_true, num_action, mu_max)
    ucb_nonudge = UCB_nonudge(theta_std, lamb, q, action, p_init, B, T, K, delta, theta_true, num_action, mu_max)
    ucb_noreveal = UCB_noreveal(theta_std, lamb, q, action, p_init, B, T, K, delta, theta_true, num_action, mu_max)
    two_agent_pd2 = Two_Agent_pd2(theta_std, lamb, q, action, p_init, B, beta_t, T, K, delta, theta_true, num_action, c, mu_max)
    two_agent_pd1 = Two_Agent_pd1(theta_std, lamb, q, action, p_init, B, beta_t, T, K, delta, theta_true, num_action, c, mu_max)
#     two_agent_pd2_context = Two_Agent_pd2_context(theta_std, lamb, q, action, p_init, B, beta_t, T, K, delta, theta_true, num_action, c, mu_max)
    
    for t in range(T):
        s_t = s_t_list[t]
        index = index_t_list[t]
        ...
        A_algorithm2, O_algorithm2, A_tilde, A_hat, o_algorithm2= two_agent_pd2.pick_action(s_t, S, Y_t0, Y_t1) #with context learning
        A_algorithm1, O_algorithm1, o_algorithm1 = two_agent_pd1.pick_action(s_t, S, Y_t0, Y_t1) #without context learning
#         A_algorithm3, O_algorithm3, o_algorithm3 = two_agent_pd2_context.pick_action(s_t, S) #with context learning

        o_pd1_list[t] = o_algorithm1
        o_pd2_list[t] = o_algorithm2
#         o_pd3_list[t] = o_algorithm3
...
#         phi_algorithm3 = two_agent_pd2_context.get_observation(A_algorithm3, s_t)
        
        # calculate expected reward and observed reward
        expected_reward_naive = np.matmul(phi_naive.T, theta_true)
        observed_reward_naive = np.random.normal(expected_reward_naive, 0.1)
        if (O_naive == 0 and Y_t0 == 0):
            barphi_naive = omniscient.Barphi(A_naive, S)
            expected_reward_naive = np.matmul(barphi_naive.T, theta_true)
        ...
        if (Y_t0 == 0):
            barphi_ucb_nonudge = omniscient.Barphi(A_ucb_nonudge, S)
        ...
        barphi_ucb_noreveal = omniscient.Barphi(A_ucb_noreveal, S)
        expected_reward_ucb_noreveal = np.matmul(barphi_ucb_noreveal.T, theta_true).item()
              
        expected_reward_algorithm2 = np.matmul(phi_algorithm2.T, theta_true)
        observed_reward_algorithm2 = np.random.normal(expected_reward_algorithm2, 0.1)
        if (O_algorithm2 == 0 and Y_t0 == 0):
            barphi_algorithm2 = omniscient.Barphi(A_algorithm2, S)
            expected_reward_algorithm2 = np.matmul(barphi_algorithm2.T, theta_true)
        if (O_algorithm2 == 1 and Y_t1 == 0):
            barphi_algorithm2 = omniscient.Barphi(A_algorithm2, S)
            expected_reward_algorithm2 = np.matmul(barphi_algorithm2.T, theta_true)

        expected_reward_algorithm1 = np.matmul(phi_algorithm1.T, theta_true)
        observed_reward_algorithm1 = np.random.normal(expected_reward_algorithm1, 0.1)
        if (O_algorithm1 == 0 and Y_t0 == 0):
            barphi_algorithm1 = omniscient.Barphi(A_algorithm1, S)
            expected_reward_algorithm1 = np.matmul(barphi_algorithm1.T, theta_true)
        if (O_algorithm1 == 1 and Y_t1 == 0):
            barphi_algorithm1 = omniscient.Barphi(A_algorithm1, S)
            expected_reward_algorithm1 = np.matmul(barphi_algorithm1.T, theta_true)
            
#         expected_reward_algorithm3 = np.matmul(phi_algorithm3.T, theta_true)
#         observed_reward_algorithm3 = np.random.normal(expected_reward_algorithm3, 0.1)
#         if (O_algorithm3 == 0): 
#             barphi_algorithm3 = omniscient.Barphi(A_algorithm3, S)
#             expected_reward_algorithm3 = np.matmul(barphi_algorithm3.T, theta_true)
        
        # update parameters
        agent.update_parameter(s_t, observed_reward_naive, phi_naive, O_naive, Y_t1, Y_t0, index)
        ucb_nonudge.update_parameter(s_t, observed_reward_ucb_nonudge, phi_ucb_nonudge, O_ucb_nonudge, Y_t1, Y_t0, index)
        ucb_noreveal.update_parameter(s_t, observed_reward_ucb_noreveal, phi_ucb_noreveal, O_ucb_noreveal, Y_t1, Y_t0, index)
        
        two_agent_pd2.update_parameter(s_t, observed_reward_algorithm2, phi_algorithm2, O_algorithm2, Y_t1, Y_t0, index)
        two_agent_pd2.update_beta_t(beta_t, t)  #update beta_t so that it is decreasing with time
        
        two_agent_pd1.update_parameter(s_t, observed_reward_algorithm1, phi_algorithm1, O_algorithm1, Y_t1, Y_t0, index)
        
#         print(two_agent_pd2_context.p_hat)
#         two_agent_pd2_context.update_parameter(s_t, observed_reward_algorithm3, phi_algorithm3, O_algorithm3, index)
#         two_agent_pd2_context.update_beta_t(beta_t, t)  #update beta_t so that it is decreasing with time
...
#         regret_algorithm3 += reward_omni - expected_reward_algorithm3
...
#         cumu_regret_algorithm3[t] = regret_algorithm3
        
    return cumu_regret_naive, cumu_regret_algorithm1, cumu_regret_algorithm2, cumu_regret_ucb_nonudge, cumu_regret_ucb_noreveal, o_pd1_list, o_pd2_list
