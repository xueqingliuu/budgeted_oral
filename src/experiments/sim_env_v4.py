...
mrt_data = pd.read_csv(os.path.join(SIM_ENV_DATA_DIR, 'oralytics_mrt_data_processed.csv'))
...
def generate_env_state(user_id, j, app_engagement):
    # user_qualities, user_actions, 
    env_state = np.ones(6)
    ...
    # change it to use user's real b_bar and a_bar
    user_ids = mrt_data['user_id'].unique()
    user_id_real = user_ids[user_id]

    user_data = mrt_data[mrt_data['user_id'] == user_id_real]
    # print(f"User {user_id_real} has {len(user_data)} data points")
    # print(f"Trying to access index {j}")
    # print(f"Available indices: {user_data.index.tolist()}")
    if len(user_data) > j:
        env_state[1] = user_data['state_b_bar'].iloc[j]
        env_state[2] = user_data['state_a_bar'].iloc[j]
        env_state[3] = user_data['state_app_engage'].iloc[j]
    else:
        env_state[1] = user_data['state_b_bar'].iloc[j % len(user_data)]
        env_state[2] = user_data['state_a_bar'].iloc[j % len(user_data)]
        env_state[3] = user_data['state_app_engage'].iloc[j % len(user_data)]
    ...
    # env_state[6] = 0
    # simulation_environment.normalize_day_in_study(1 + (j // 2))
...
def get_logistic_app_open_probs(user_id, state):
    ...
    Args:
        ...
        state: environment state vector [tod, b_bar, a_bar, app_engage,  bias,]
...
BASE_BERN_NAMES = [
    'state_tod.Base',
    'state_b_bar.norm.Base', 'state_a_bar.norm.Base',
    'state_app_engage.Base', 
    'state_day_type.Base',
    'state_bias.Base'
    # ,
    # 'state_day_in_study.Base'
]
# BASE_POISSON_NAMES = [
...
#   poisson_base = np.zeros(len(bern_base))
...
def get_adv_params_for_user(user):
    ...
    for action_type in ACTION_TYPES:
        if action_type == 'Base':
            bern_params = np.zeros(6)
        else:
            bern_names = [
                f'state_tod.{action_type}',
                f'state_b_bar.norm.{action_type}', f'state_a_bar.norm.{action_type}',
                f'state_app_engage.{action_type}', 
                f'state_day_type.{action_type}',
                f'state_bias.{action_type}'
                # f'state_day_in_study.{action_type}'
            ]
            # poisson_names = [
    ...
    # poisson_adv_all = np.zeros(len(bern_adv_all))
    
    # Return shape: (5, 5) for 5 actions x 5 state features
    return np.array(bern_adv_all)
...
def turn_alg_state_to_env_state(state):
    # x_4_mean = 2/7
    # x_6_mean = 0
    env_state = np.array([state[0], state[1], state[2], state[3], state[4], 1]) #, state[6]])
...
def create_user_envs_under_no_advantage(users_list, state=None, no_adv_all_states=False):
    ...
    for i, user_id in enumerate(users_list):
      ...
      if no_adv_all_states:
          adv_params = np.zeros(6), np.zeros(6) #, np.zeros(7)
      else:
      ...
      user_effect_func_bern = get_user_effect_funcs()
      new_user = UserEnvironmentV4(user_id, model_type, adv_params, base_params, user_effect_func_bern)
      all_user_envs[i] = new_user
...
class SimulationEnvironmentV4(simulation_environment.SimulationEnvironmentAppEngagement):
    ...
    def generate_current_state(self, user_idx, j):
        ...
        return generate_env_state(user_idx, j, prior_app_engagement)
